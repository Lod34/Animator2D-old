...
NUM_EPOCHS = 250
LR = 2e-4
IMG_SIZE = 128  # Riduciamo la dimensione per semplicità
...
class SimpleGenerator(nn.Module):
    def __init__(self, text_embedding_dim, img_size=IMG_SIZE, channels=3):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(256, 8 * 8 * 128), # Adjust the output features to match the desired image dimensions
            nn.ReLU()
        )
        # Rete di convoluzioni trasposte per generare l'immagine finale (128x128)
        self.conv = nn.Sequential(
            ...
            # Add more layers to upscale to 128x128
            nn.ConvTranspose2d(channels, channels, kernel_size=4, stride=2, padding=1),  # Output: 128x128
...
torch.save(model.state_dict(), "Animator2D-mini-250e.pth")
